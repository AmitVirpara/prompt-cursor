...
- Implement the `disp_trans_sql.sql` migration service using `disp_trans_sql_mapping.md`, `disp_trans_migration_support.sql`, and `disp_trans_migration_support_data.sql`. It must load legacy `iwaytransca` rows from staging, write crosswalks to `legacy_entity_mappings`, convert dispatch transactions into canonical order, trip, event, equipment, driver, payment, statement, document, and tracking models, and capture bad rows in `legacy_rejected_rows`.
